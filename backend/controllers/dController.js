...
exports.insertOrUpdateD =  [ upload.single('image'),(req, res) => {
  const { 
    firstName, 
    lastName, 
    dName, 
  ...
  const query = `
    INSERT INTO d_info (
      first_name, last_name, d_name, post, image_url, 
  ...
  const values = [
    firstName, lastName, dName, post, imageUrl, 
    description_1, list_1, 
    description_2, list_2, 
    description_3, list_3
  ];
...
// Mettre à jour un DG
exports.updateD = [upload.single("image"), (req, res) => {
  const { id } = req.params;
  const {
    firstName,
    lastName,
